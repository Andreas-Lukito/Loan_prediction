...
with st.sidebar:
    selected_preset = st.session_state.current_preset
...
education = st.selectbox("Education", ["High School", "Associate", "Bachelor", "Master", "Doctorate"], key = "education")
home_ownership = st.selectbox("Home Ownership", ["Rent", "Own", "Mortgage", "Other"], key = "home_ownership")
loan_intent = st.selectbox("Loan Intent", ["Education", "Medical", "Venture", "Personal", "Home Improvement", "Debt Consolidation"], key = "loan_intent")
previous_loans = st.selectbox("Previous Loan Default", ["Yes", "No"], key = "previous_loans")

# Ensure all numerical inputs have consistent formatting
income = st.number_input("Annual Income ($)", min_value=0.0, key = "income")
loan_amount = st.number_input("Loan Amount ($)", min_value=0.0, key = "loan_amount")
loan_rate = st.number_input("Interest Rate (%)", min_value=0.0, key = "loan_rate")
credit_score = st.number_input("Credit Score", min_value=0, key = "credit_score")
credit_hist_length = st.number_input("Credit Duration (in one year)", min_value=0, key = "credit_hist_length")
person_emp_exp = st.number_input("Work Experience (in years)", min_value=0.0, key = "person_emp_exp")
